 ...
 */
export function analyzeDeck(cardNames: string[]): DeckAnalysis {
  ...
  const checkResults: DeckAnalysis['checkResults'] = {};
  ...
  const winConditions = cards.filter(c => c.roles.includes(CARD_ROLES.WIN_CONDITION));
  ...
  const smallSpells = cards.filter(c => c.roles.includes(CARD_ROLES.SPELL_SMALL));
  const bigSpells = cards.filter(c => c.roles.includes(CARD_ROLES.SPELL_BIG));
  ...
  // Check 4: Air Defense
  const airDefense = cards.filter(c => 
    c.roles.includes(CARD_ROLES.AIR_DEFENSE) || 
    c.targets === 'both' || 
    c.targets === 'air'
  );
  ...
  // Check 5: Tank Killer
  const tankKillers = cards.filter(c => c.roles.includes(CARD_ROLES.TANK_KILLER));
  const buildings = cards.filter(c => c.roles.includes(CARD_ROLES.BUILDING));
  ...
  const redundantRoles = Object.entries(roleCount)
    .filter(([role, count]) => count > 3 && role === CARD_ROLES.SPLASH)
    .map(([role]) => role);
